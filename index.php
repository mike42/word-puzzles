...
if(has('submit')) { /* Everything which involves a submit */
	if(has('height') && has('width') && has('lang') && has('diagonal') && has('reverse')) {
		...
		if(has('word_list')) {
			...
			if(count($req_word_list_arr) == 1) { $req_word_list_arr = explode(",",take('word_list')); }
			/* Process list. Trim and remove blanks */
			foreach($req_word_list_arr as $key => $val) {
				$req_word_list_arr[$key] = trim($req_word_list_arr[$key]);
				if($req_word_list_arr[$key] == "") { unset($req_word_list_arr[$key]); }
			}
...
function has($var)  { return isset($_REQUEST[$var]); }
function take($var) { return stripslashes($_REQUEST[$var]); }
?>
