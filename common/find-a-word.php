...
class cls_find_a_word {
	...
	function load_words($words = Array(), $num = 0) {
		...
		if(count($words) == 0) {
			$i = 1;
			if($num < 0) { $num = 0; }
			if(count($this -> dictionary) == 0) { return false; }
			while($num > ($i - 1)) {
	...
	function calculate($lang_code) {
		...
		if($this -> diagonal == "never") 		{ $d = 1; }
		if($this -> diagonal == "sometimes")	{ $d = -1; }
		$r = 0; /* Half-and-half reverse */
		if($this -> reverse == "never") 		{ $r = 1; }
		if($this -> reverse == "sometimes")		{ $r = -1; }
		...
		foreach($this -> words as $word) {
			...
			if($d == 0) { $dir = rand(0,2); $c1 = 3; } else { $dir = rand(0,1); $c1 = 2; }
			if($r == 0) { $rev = rand(0,1); $c2 = 2; } else { $rev = 0; $c2 = 1; }
			...
			while($fail != 0 && $loop < $comb) {
				if($fail > 0) {
					...
					if($d != 1 && $comb % $c2 == 0) { $dir++; } /* Flip if allowed but wait for $rev if it has 2 modes. */
					if($dir == 3) { $dir = 0; }
					if($r != 1) { $rev++; }
					if($rev == 2) { $rev = 0; }
				}
				...
				$loop++;
				if($abort) { break; } /* Part of optional fast-bug-out mode */
			}
			if($abort) { break; } /* Part of optional fast-bug-out mode */
		}
		...
		for($y = 1; $y <= $this -> height; $y++) {
			for($x = 1; $x <= $this -> width; $x++) {
				...
				if($possible != 0) {
					...
					if($this -> fast) { return $possibilities; }
				}
			}	
		}
